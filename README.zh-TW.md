...
「NameIO Stream」進階選項僅在建立新磁碟區時選擇 nameio/stream 檔案名稱加密。既有磁碟區會繼續使用 `.encfs6.xml` 中儲存的設定。
...
選項:
  ...
  --nameio-stream                              建立新磁碟區時使用 nameio/stream 檔案名稱加密
                                               (預設: nameio/block)
...
範例:
  ...
  encfs.exe C:\Data M: --nameio-stream                     # 使用 nameio/stream 建立新磁碟區
