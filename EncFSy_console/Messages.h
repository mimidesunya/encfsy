...
inline const char* GetUsageText_JA() {
    return
        ...
        u8"  --nameio-stream                              新規作成時にファイル名暗号化を nameio/stream にする\n"
        u8"                                               （既定: nameio/block）\n"
        u8"  --cloud-conflict                             クラウドコンフリクトファイル対応を有効化\n"
        u8"                                               （Dropbox、Google Drive、OneDrive）。既定では無効\n"
        u8"  --reverse                                    逆モード: 平文の rootDir を暗号化してマウントPoint に表示\n"
        u8"\n"
        u8"例:\n"
        ...
        u8"  encfs.exe C:\\Data M: --nameio-stream                    # nameio/stream で新規作成\n"
        u8"\n"
...
inline const char* GetUsageText_KO() {
    return
        ...
        u8"  --nameio-stream                              새 볼륨 생성 시 nameio/stream 파일 이름 인코딩 사용\n"
        u8"                                               (기본값: nameio/block)\n"
        u8"  --cloud-conflict                             클라우드 충돌 파일 처리 활성화\n"
        u8"                                               (Dropbox, Google Drive, OneDrive). 기본값 비활성화\n"
        ...
        u8"  encfs.exe C:\\Data M: --nameio-stream                     # nameio/stream으로 새 볼륨 생성\n"
        u8"\n"
...
inline const char* GetUsageText_ZH() {
    return
        ...
        u8"  --nameio-stream                              新建卷时使用 nameio/stream 文件名编码\n"
        u8"                                               (默认: nameio/block)\n"
        u8"  --cloud-conflict                             启用云冲突文件处理（Dropbox、Google Drive、\n"
        u8"                                               OneDrive）。默认禁用\n"
        ...
        u8"  encfs.exe C:\\Data M: --nameio-stream                     # 使用 nameio/stream 新建卷\n"
        u8"\n"
...
inline const char* GetUsageText_ZH_TW() {
    return
        ...
        "\n"
        u8"選項:\n"
        u8"  -u <mountPoint>                              卸載指定的磁碠區\n"
        u8"  -l                                           列出目前掛載的 Dokan 磁碟區\n"
        u8"  -v                                           將除錯輸出傳送到偵錯工具\n"
        u8"  -s                                           將除錯輸出傳送到 stderr\n"
        u8"  -i <ms>              (預設: 120000)          逾時時間（毫秒），逾時後操作中止並卸載磁碠區\n"
        ...
        u8"  --nameio-stream                              新建磁碟區時使用 nameio/stream 檔案名稱編碼\n"
        u8"                                               (預設: nameio/block)\n"
        u8"  --cloud-conflict                             啟用雲端衝突檔案處理（Dropbox、Google Drive、\n"
        u8"                                               OneDrive）。預設為停用\n"
        ...
        u8"  encfs.exe C:\\Data M: --nameio-stream                     # 使用 nameio/stream 新建磁碟區\n"
        u8"\n"
...
inline const char* GetUsageText_RU() {
    return
        ...
        u8"  --nameio-stream                              При создании тома использовать nameio/stream\n"
        u8"                                               для кодирования имён (по умолч.: nameio/block)\n"
        u8"  --cloud-conflict                             Включить обработку конфликтных файлов облака\n"
        u8"                                               (Dropbox, Google Drive, OneDrive). По умолч. выкл.\n"
        ...
        u8"  encfs.exe C:\\Data M: --nameio-stream                     # Создать том с nameio/stream\n"
        u8"\n"
...
inline const char* GetUsageText_AR() {
    return
        ...
        u8"  --use-credential                             قراءة كلمة مرور من مدير بيانات الاعتماد\n"
        u8"                                               (الاحتفاظ بكلمة المرور)\n"
        u8"  --use-credential-once                        قراءة كلمة مرور من مدير بيانات الاعتماد\n"
        ...
        u8"  --dokan-enable-unmount-network-drive         السماح بإلغاء تركيب محرك الشبكة عبر المست explorer\n"
        ...
        u8"  --nameio-stream                              استخدام nameio/stream لأسماء الملفات عند إنشاء وحدة جديدة\n"
        u8"                                               (الافتراضي: nameio/block)\n"
        u8"  --cloud-conflict                             تمكين معالجة ملفات التعارض السحابية\n"
        u8"                                               (Dropbox، Google Drive، OneDrive). معطل افتراضياً\n"
        ...
        u8"  encfs.exe C:\\Data M: --nameio-stream                     # إنشاء وحدة جديدة باستخدام nameio/stream\n"
        u8"\n"
...
inline const char* GetUsageText_DE() {
    return
        ...
        u8"  --nameio-stream                              Beim Erstellen nameio/stream für Dateinamen verwenden\n"
        u8"                                               (Standard: nameio/block)\n"
        u8"  --cloud-conflict                             Cloud-Konfliktdatei-Behandlung aktivieren\n"
        u8"                                               (Dropbox, Google Drive, OneDrive). Standardmäßig aus\n"
        ...
        u8"  encfs.exe C:\\Data M: --nameio-stream                     # Neues Volume mit nameio/stream erstellen\n"
        u8"\n"
