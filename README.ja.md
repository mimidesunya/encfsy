...
詳細オプションの「NameIO Stream」は、新規ボリューム作成時だけファイル名暗号化を nameio/stream にします。既存ボリュームは `.encfs6.xml` に保存された設定を使い続けます。
...
オプション:
  ...
  --nameio-stream                              新規作成時にファイル名暗号化を nameio/stream にする
                                               （既定: nameio/block）
...
例:
  ...
  encfs.exe C:\Data M: --nameio-stream                    # nameio/stream で新規ボリュームを作成
