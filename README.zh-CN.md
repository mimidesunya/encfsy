...
"NameIO Stream"高级选项仅在创建新卷时选择 nameio/stream 文件名加密。现有卷会继续使用 `.encfs6.xml` 中保存的设置。
...
选项:
  ...
  --nameio-stream                              创建新卷时使用 nameio/stream 文件名加密
                                               (默认: nameio/block)
...
示例:
  ...
  encfs.exe C:\Data M: --nameio-stream                     # 使用 nameio/stream 创建新卷
